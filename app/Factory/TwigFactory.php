...
class TwigFactory {
	...
	public function __invoke( Config $config, Router $router, Messages $messages, Helper $session ) {
		$manifest      = [];
		...
		if ( $config->get( 'i18n.enable' ) ) {
			...
			$twig->getEnvironment()->addFunction( new TwigFunction( 'lang_enable_name', function () use ( $localeApp, $config ) {
